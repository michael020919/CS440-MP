...
class Maze:
    def __init__(self, alien, walls, waypoints, goals, move_cache={}, k=5, enable_heuristic=True):
        ...
        self.__start = MazeState(self.__start, self.get_objectives(), 0, self, self.enable_heuristic)
    ...
    def filter_valid_waypoints(self):
        """Filter valid waypoints on each alien shape.
            Since the initial waypoints are randomly generated, some of them are likely to be invalid.
            This function should filter out the invalid waypoints for each alien shape.
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        """Find the k nearest valid neighbors to the cur_waypoint from a list of 2D points.
            ...
            Return:
                the k valid waypoints that are closest to waypoint whose shape is cur_shape
        """
        nearest_neighbors = []
    ...
    def get_neighbors(self, x, y, shape_idx):
        ...
        min_shape_idx = max(shape_idx -1, 0)
        max_shape_idx = min(shape_idx+1, 2)
        for end in [(x, y, min_shape_idx), (x, y, max_shape_idx)]:
            start = (x, y, shape_idx)
