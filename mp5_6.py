...
class Application:
    ...
    def execute(self, granularity, trajectory):
        ...
        if not self.__human:
            ...
            try:
                t1 = time.time()
                start_state = MazeState((*self.alien.get_centroid(), self.alien.get_shape_idx()), self.goals, 0, maze, enable_heuristic=True)
                path = astar_search(start_state)
                t2 = time.time()
            ...
            if path is None:
                ...
                self.trajectory = [x.current_state for x in path]
...
if __name__ == "__main__":
    ...
    app.execute(args.granularity, args.trajectory)
